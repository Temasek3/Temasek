import type {
  ...
  TemasekWebApiFeaturesRoomsScheduleContractsScheduleActivityDto as ScheduleActivityContract,
} from '~/kubb'
...
export type ScheduleActivity = Required<Pick<ScheduleActivityContract, RequiredActivityKeys>>

export type RoomScheduleSnapshot = Omit<Required<RoomScheduleSnapshotContract>, 'schedule' | 'updatedAtUtc'> & {
...
export function normalizeSchedule(value: unknown): ScheduleActivity[] {
  if (!Array.isArray(value)) {
    return []
  }

  return value.map((activity, index) => normalizeActivity((activity ?? {}) as Partial<ScheduleActivityContract>, index))
}

export function normalizeActivity(activity: Partial<ScheduleActivityContract>, index: number): ScheduleActivity {
...
export function sortSchedule(value: ScheduleActivity[]) {
...
export function formatEndsIn(activity: ScheduleActivity | null, currentTime: Date) {
  if (!activity) {
    return ''
  }
...
export function getScheduleState(schedule: ScheduleActivity[], currentTime: Date) {
  const currentMinute = startOfMinute(currentTime)
  let current: ScheduleActivity | null = null
...
export function getActivitiesAfterNext(schedule: ScheduleActivity[], nextActivity: ScheduleActivity | null) {
  if (!nextActivity) {
    return []
  }
...
export function getMetaItems(activity: ScheduleActivity): ScheduleMetaItem[] {
  return [
...
export function getSeatingAlertMessage(nextActivity: ScheduleActivity | null, currentTime: Date) {
...
export function validateScheduleArray(value: unknown) {
  ...
  const normalized = value.map((item, index) => normalizeActivity((item ?? {}) as Partial<ScheduleActivityContract>, index))
...
export function getSuggestedStartTime(sourceSchedule: ScheduleActivity[]) {
